# LingOrm × Dior AW2026 Caption Generator

Luxury-minimal caption generator designed for coordinated global support of  
**LingOrm as Dior Brand Ambassadors** during Paris Fashion Week AW2026.

Created by **the_wanderer16**

---

## 🌍 Live Site

https://lingorm-caption-generator.pages.dev

---

# ✨ Purpose

This tool was built to:

- Standardize support posts across X, Threads, Instagram, and Facebook  
- Ensure correct hashtag usage for global monitoring (Lefty / EMV tracking)  
- Maintain consistent mention structure for Dior and LingOrm  
- Reduce caption duplication across multiple users  
- Support multi-day trending coordination (Airport + PFW day)  
- Make coordinated posting easier on both desktop and mobile  

This is a static tool.  
No data is collected. No captions are stored remotely.

---

# 🖤 Core Principle

All posts should:

- Maintain alignment with Dior’s image  
- Support global data tracking via consistent hashtag usage  
- Maintain LingOrm as a unit representation under Dior  

---

# 🗓 Dual Mode Caption Engine

The generator includes Trend Day Mode Selection:

## 🔹 2 March — Airport Looks Mode

Designed for airport arrival / departure / transit content.

Focus:
- Travel elegance
- Arrival momentum
- Street-style impact
- Paris arrival narrative

Still supports #PFW tracking where relevant.

---

## 🔹 3 March — PFW Show Day Mode

Designed for Dior AW26 runway / official appearance content.

Focus:
- Couture language
- Dior Autumn Winter 2026 positioning
- Ambassador authority statements
- Brand alignment

Switching the dropdown automatically loads the correct phrase bank.

---

# 🧭 Platform Modes

The generator supports:

---

## 1️⃣ X (Posts)

- Enforced 280 character limit  
- Optimized for stacked hashtags  
- CTA automatically disabled  

---

## 2️⃣ Threads (Posts)

- Similar to X  
- Allows CTA  
- Uses primary hashtag block  

---

## 3️⃣ Threads (Replies / Quotes) ⚠ CRITICAL MODE

This mode enforces official coordination guidelines.

Required in every reply / quote:

- #LingOrmDiorAW26  
- #pfw  
- @Dior  
- @linglingkwong  
- @orm.kornnaphat  

If these are missing from the Threads Reply block, generation will stop.

This ensures:

- Proper Paris Fashion Week tagging  
- Accurate monitoring (Lefty / Firehose-weighted systems)  
- Full brand visibility  

---

## 4️⃣ Instagram (Posts)

- Multi-line stacked hashtag formatting  
- CTA enabled  
- Supports @dior tagging  

---

## 5️⃣ Facebook (Posts)

- Optimized stacked formatting  
- CTA enabled  

---

# 🏷 Hashtag Blocks (Editable)

The “Hashtag blocks” section is always visible.

You may edit:

- X post block  
- Instagram post block  
- Facebook post block  
- Threads Post / Reply / Quote block  

⚠ Editing the Threads Reply block must still include required tags.

‼️ Always update blocks according to LingOrm_TagTrend instructions.

---

# 🧠 Caption Engine Logic

Each caption is generated using:

- Subject  
- Fashion descriptor  
- Anchor (Airport or PFW context)  
- Authority statement (as Dior Brand Ambassadors)  
- Optional ending line  

Randomized structure templates:

- Sentence + separate ending  
- Combined single paragraph  
- Ending first, then statement  

Occasionally injects full names:

Lingling Kwong and Orm Kornnaphat

This improves monitoring coverage and SOV tracking.

---

# 🔁 Duplicate Detection

Two levels of duplicate control:

---

## 1️⃣ Within Current Batch

If the same caption appears twice in one generation:

It will be marked DUPLICATE.

---

## 2️⃣ Seen Before (This Device Only)

Previously generated captions are stored locally in your browser.

If a new caption matches an older one:

It will be marked SEEN.

You may clear history anytime using:

Clear duplicate history (this device)

No server storage is used.

---

# 🗂 Phrase Banks (Embedded)

The generator uses:

- Airport Phrase Bank (2 Mar mode)
- PFW Phrase Bank (3 Mar mode)

These banks are embedded directly in the source code.

There is no public BANK editor to prevent formatting errors or misuse.

If updates are required:
- The creator updates the source code
- A new version is deployed

---

# 📏 Posting Guidelines

## Primary Topic

Always use:

#LingOrmDiorAW26

This ensures global monitoring accuracy.

---

## Paris Fashion Week Context

Include:

#pfw

Especially in replies and quotes.

---

## Tagging

Always mention:

@Dior  
@linglingkwong  
@orm.kornnaphat  

For maximum visibility and brand alignment.

Remember to manually tag correctly on each platform.

---

# 🎯 Best Practices

- Generate smaller batches (10–20 at a time)  
- Avoid overposting identical structures  
- Switch modes appropriately (Airport vs PFW day)  
- Keep tone elegant and brand-aligned  
- Always update Hashtag Blocks according to LingOrm_TagTrend updates  

---

# 🔒 Privacy

This is a static Cloudflare Pages site.

- No analytics  
- No tracking  
- No caption uploads  
- No personal data stored  

LocalStorage is used only for duplicate detection on your device.

The GitHub repository is private.  
Only the deployed static site is publicly accessible.

---

# 🛠 Tech Stack

- HTML  
- CSS (Luxury minimal theme)  
- Vanilla JavaScript  
- Cloudflare Pages deployment  
- Private GitHub version control  

---

# 💜 For the Fandom 💜

This tool exists to support LingOrm responsibly and strategically for Paris Fashion Week.

Coordinated support amplifies impact.  
Consistency strengthens brand positioning.  

Every well-crafted caption strengthens LingOrm’s visibility, narrative, and digital impact during Paris Fashion Week AW2026.

Thank you for contributing to LingOrm’s Dior AW2026 campaign.

Let’s elevate LingOrm’s impact together.

---

# Creator

the_wanderer16  
LingOrm × Dior AW2026 Campaign

---

If you fork or adapt this tool, please maintain credit and respect coordinated posting guidelines.